<?php
$pageTitle='Student Login'; require 'config.php'; require 'includes/functions.php';
...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 if($u && password_verify($password,$u['password_hash'])){
   if($u['role']!=='student'){ flash('error','This account is not a Student account. Please use the correct Unimesh site.'); }
...
<div class="form-card card"><div class="kicker">🎓 Student Site</div><h1>Student Login</h1><p class="muted">This login is independent from the other Unimesh gateways.</p><form method="post"><div class="form-group"><label>Email</label><input type="email" name="email" required></div><div class="form-group"><label>Password</label><input type="password" name="password" required></div><button class="btn">Login as Student</button></form></div>
